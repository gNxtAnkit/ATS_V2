...
from gnxthire_identity.api.dependencies import (
    platform_admin_identity_service,
...
from gnxthire_identity.platform_admin.schemas import (
    PlatformAdminForgotPasswordRequest,
    PlatformAdminLoginRequest,
    PlatformAdminRequestEmailVerificationRequest,
    PlatformAdminResetPasswordRequest,
    PlatformAdminVerifyEmailRequest,
)
...
router = APIRouter(prefix="/v1/identity/platform-admin", tags=["identity-platform-admin"])
...
def _service_from_authenticated(repository, settings) -> PlatformAdminIdentityService:
...
def login(
    payload: PlatformAdminLoginRequest,
    metadata: RequestMetadata = Depends(request_metadata),
    service: PlatformAdminIdentityService = Depends(platform_admin_identity_service),
) -> LoginResponse:
...
def forgot_password(
    payload: PlatformAdminForgotPasswordRequest,
    metadata: RequestMetadata = Depends(request_metadata),
    service: PlatformAdminIdentityService = Depends(platform_admin_identity_service),
) -> MessageResponse:
...
def reset_password(
    payload: PlatformAdminResetPasswordRequest,
    metadata: RequestMetadata = Depends(request_metadata),
    service: PlatformAdminIdentityService = Depends(platform_admin_identity_service),
) -> MessageResponse:
...
def request_email_verification(
    payload: PlatformAdminRequestEmailVerificationRequest,
    metadata: RequestMetadata = Depends(request_metadata),
    service: PlatformAdminIdentityService = Depends(platform_admin_identity_service),
) -> MessageResponse:
...
def verify_email(
    payload: PlatformAdminVerifyEmailRequest,
    metadata: RequestMetadata = Depends(request_metadata),
    service: PlatformAdminIdentityService = Depends(platform_admin_identity_service),
) -> MessageResponse:
