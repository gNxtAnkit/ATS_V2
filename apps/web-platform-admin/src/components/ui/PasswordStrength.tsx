...
const defaultPolicy: PasswordPolicyResponse = {
  min_length: 12,
  max_length: 128,
...
function getPasswordRules(policy: PasswordPolicyResponse): Rule[] {
  return [
...
const strengthColors = ['', 'bg-red-500', 'bg-amber-500', 'bg-yellow-400', 'bg-emerald-500', 'bg-emerald-500'];
const strengthTextColors = ['', 'text-red-600', 'text-amber-600', 'text-yellow-600', 'text-emerald-600', 'text-emerald-600'];
        ...
        <div className="space-y-1.5">
          <div className="flex gap-1">
            {[1, 2, 3, 4].map((index) => (
              <div
                key={index}
                className={['h-1 flex-1 rounded-full transition-all duration-300', strength > index ? strengthColors[strength] : 'bg-slate-200'].join(' ')}
              />
            ))}
          </div>
          <p className="text-xs text-slate-500">
            Strength: <span className={['font-semibold', strengthTextColors[strength]].join(' ')}>{strengthLabels[strength]}</span>
          </p>
        </div>
      )}
        ...
        {rules.map((rule) => {
          ...
          return (
            <li key={rule.label} className="flex items-center gap-2">
              ...
              <span className={['text-xs transition-colors', passed ? 'text-slate-700' : 'text-slate-500'].join(' ')}>{rule.label}</span>
            </li>
          );
        })}
        {showMatch && (
          <li className="flex items-center gap-2">
            ...
            <span className={['text-xs transition-colors', passwordsMatch ? 'text-slate-700' : 'text-slate-500'].join(' ')}>Passwords match</span>
          </li>
        )}
      </ul>
    </div>
